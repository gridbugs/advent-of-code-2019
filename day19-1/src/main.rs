...
impl Solution {
    ...
    fn test_coord_is_pulled(&mut self, x: i128, y: i128) -> bool {
        let mut computer = IntcodeComputer::new(&self.program);
        self.input_buffer.write(x);
        self.input_buffer.write(y);
        loop {
            match computer.run(&mut self.input_buffer, &mut self.output_buffer) {
                StopStatus::WroteOutput => {
                    return self.output_buffer.read().unwrap() != 0;
                }
    ...
    fn run(&mut self) -> u32 {
        ...
        for y in 0..50 {
            for x in 0..50 {
                count += self.test_coord_is_pulled(x, y) as u32;
            }
        }
        count
    }
