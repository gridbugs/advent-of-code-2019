...
impl Map {
    ...
    fn solve_single(&self, start: Coord) -> u32 {
        let interesting_coord_keys = self
            .reachable_keys_from(start)
            .iter()
            .map(|ReachableKey { coord, .. }| coord)
            .chain([start].iter())
        ...
        })) = queue.pop()
        {
            ...
            if seen_set.insert((coord, key_set)) {
                let next_nodes = interesting_coord_keys[&coord]
                    ...
                    .filter(|ReachableKey { door_set, .. }| (door_set & all_keys) & !key_set == 0)
                    .map(|key| {
                        Reverse(SearchNode {
    ...
    fn solve(&self) -> u32 {
        self.start
            .iter()
            .map(|&start| self.solve_single(start))
            .sum()
    }
}
